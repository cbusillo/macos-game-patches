...
run_dir="$run_root/native-bind-$timestamp"
...
cxstart="$crossover_app/Contents/SharedSupport/CrossOver/bin/cxstart"
...
client_binary="$build_dir/d3d11_dxvk_iosurface_probe.exe"
staged_client="$bottle_probe/d3d11_dxvk_iosurface_probe.exe"
staged_bridge="$bottle_probe/alvr_iosurface_bridge.dll"
service_name="com.alvr.handoff.bind-probe.$UID.$$"
nonce="$(date +%s)$$"
server_pid=""
...
cleanup() {
	...
	rm -f "$bottle_probe"/d3d11_dxvk_iosurface_probe_{d3d11,dxgi}.log
	exit "$exit_status"
}
trap cleanup EXIT INT TERM

for path in "$wine_source" "$wine_build" "$cxstart" "$moltenvk"; do
	if [[ ! -e "$path" ]]; then
...
x86_64-w64-mingw32-g++ -O2 -g -std=c++20 \
	...
	"$repo_root/tools/d3d11_dxvk_iosurface_probe.cpp" \
	-ld3d11 -ldxgi -lole32 \
	-o "$client_binary"
...

{
	...
	shasum -a 256 \
		...
		"$repo_root/tools/run_dxvk_native_iosurface_bind_probe.sh" \
		"$server_binary" \
		"$client_binary" \
...
"$server_binary" server-bind "$service_name" "$nonce" \
...
"$cxstart" \
	...
	--no-gui \
	--wait \
	--env "CX_GRAPHICS_BACKEND=dxvk WINEDLLPATH=$bridge_root ALVR_MOLTENVK_PATH=$moltenvk WINEDEBUG=-all DXVK_LOG_LEVEL=info" \
	'C:\alvr-probes\d3d11_dxvk_iosurface_probe.exe' \
	--width 16 \
	--height 8 \
	--native-surface-service "$service_name" \
	--native-surface-nonce "$nonce" \
...
for log in "$bottle_probe"/d3d11_dxvk_iosurface_probe_{d3d11,dxgi}.log; do
	if [[ -f "$log" ]]; then
		cp "$log" "$run_dir/"
	fi
done
...
if [[ $client_status -eq 0 && $server_status -eq 0 ]] &&
	rg -q 'bridge_import_bind .*result=pass' "$run_dir/client.log" &&
	rg -q 'bridge_signal_ready .*result=pass' "$run_dir/client.log" &&
	rg -q 'native_surface_bound=true' "$run_dir/client.log" &&
	rg -q 'native_read .*result=pass' "$run_dir/server.log"; then
