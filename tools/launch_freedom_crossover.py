...
def hide_windows(patterns: tuple[str, ...], verbose: bool) -> None:
    script = f'''
-- {SCRIPT_MARKER}
set targetPatterns to {applescript_list(patterns)}
...
tell application "System Events"
    repeat with proc in application processes
        try
            ...
            if my textMatchesAny(processName, targetPatterns) then
                ...
                repeat with win in windows of proc
                    ...
                    if my textMatchesAny(windowName, targetPatterns) then
                        try
                            set miniaturized of win to true
                        end try
                        try
