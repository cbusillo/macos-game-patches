...
- Target Python 3.13+; use type hints everywhere.
...
- Keep I/O at the edges; separate pure logic from filesystem/process calls when possible.
- When adding commands/entrypoints, ensure `src/` is on `sys.path` (see existing CLIs).
- Update related docs when code structure or behavior changes (README, per-game docs, TECHNICAL).
