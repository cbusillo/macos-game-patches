...
Ask for headset time only after disconnecting the AVP from the Windows ALVR
server so it can attach to the macOS bridge. Keep the current Freedom CrossOver
producer and macOS bridge running, then have the user reconnect the AVP client
and compare the first Freedom scene against the Windows baseline: immersive
360-degree environment, world-locked feel, stereo alignment, and any crop or
depth discomfort.
